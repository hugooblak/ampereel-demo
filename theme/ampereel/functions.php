<?php
/**
 * Ampére El AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Ampére El AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through am_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'ampereel', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'ampereel' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'ampereel' ) ) );

		register_block_pattern_category( 'ampereel-sections', array( 'label' => __( 'Ampére El AB sections', 'ampereel' ) ) );
		register_block_pattern_category( 'ampereel-pages', array( 'label' => __( 'Ampére El AB page layouts', 'ampereel' ) ) );
	}
);
