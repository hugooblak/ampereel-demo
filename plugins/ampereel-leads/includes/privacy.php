...
add_filter(
	...
	function ( $exporters ) {
		$exporters['ampereel-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'ampereel-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'ampereel-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'ampereel-leads',
						'group_label' => __( 'Quote requests', 'ampereel-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['ampereel-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'ampereel-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
