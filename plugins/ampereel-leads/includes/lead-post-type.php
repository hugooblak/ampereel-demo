...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'ampereel-leads' ),
					'singular_name' => __( 'Lead', 'ampereel-leads' ),
					'edit_item'     => __( 'Förfrågan', 'ampereel-leads' ),
					'search_items'  => __( 'Search leads', 'ampereel-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'ampereel-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'ampereel-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'ampereel-leads' ),
			'npl_phone'    => __( 'Phone', 'ampereel-leads' ),
			'npl_service'  => __( 'Needs', 'ampereel-leads' ),
			'npl_timeline' => __( 'When', 'ampereel-leads' ),
			'npl_zip'      => __( 'ZIP', 'ampereel-leads' ),
			'npl_source'   => __( 'Source', 'ampereel-leads' ),
			'npl_status'   => __( 'Status', 'ampereel-leads' ),
			'date'         => __( 'Received', 'ampereel-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'ampereel-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'ampereel-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'ampereel-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'ampereel-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'ampereel-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'ampereel-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'ampereel-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'ampereel-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'ampereel-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'ampereel-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'ampereel-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'ampereel-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
