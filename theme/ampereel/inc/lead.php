 ...
 * or photo. They call am_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Ampére El AB.
 *
 ...
 * @return string
 */
function am_sample_note() {
	return am_lead( 'ar_riktig' ) ? '' : ' <span class="am-sample">(platshållare)</span>';
}
 ...
 * @return string File name for am_lead_img().
 */
function am_lead_bild( $i ) {
	$bilder = (array) am_lead( 'bilder', array() );
 ...
 * @return string
 */
function am_ring_label() {
	$namn = am_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, am_lead( 'telefon' ) ) : 'Ring ' . am_lead( 'telefon' );
}
